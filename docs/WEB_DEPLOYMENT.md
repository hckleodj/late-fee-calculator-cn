...
## 轻量本地优先测试版（2026-08-02）

- 测试地址：<https://dajin-car-loan-v09-d5c3yb395b1ee-1461974653.tcloudbaseapp.com/lite/>
- 独立分支：`feat/cloud-webapp-local-first-lite`
- 前端继续使用原 GitHub Pages 单页布局；方案测算、倒推和滞纳金全部本地计算。
- 客户、合同、收款和删除先写 `localStorage`，再通过持久化 outbox 异步同步。
- 页面打开、联网恢复、回到前台和重新进入时会自动重试；普通保存只发一次 `sync.push`。
- 同步工作区继续使用现有 `app_settings` 和 `audit_logs`，没有新增集合、云资源或付费项。
- GitHub Pages 旧版和当前云端根页面均未覆盖。网页关闭后不能像原生 App 一样持续后台运行。

自动测试 36 项通过；线上已验证新增、修改、刷新回读、删除、离线保存和恢复网络自动重试。线上性能跟踪 LCP 为 543 ms，方案测算和滞纳金输入过程中没有新增 API 请求。真实微信物理设备和两台独立设备仍需最终人工验收。
...
2026-08-01 首次上线完成核心验证：线上完成正常还款、逾期部分还款和中途结清；旧网页备份完成预览、首次导入和重复拦截；第二个独立浏览器会话能读取同一份云端数据。

性能修复版的本地自动化测试为 24 项：旧数据导入改为每次保存 1 位客户并返回进度，可在超时或网络中断后继续；看板只查询截至未来 3 天的未结清期次；网页对短时间重复浏览做内存缓存，客户详情不再额外请求一次结清接口。部署后必须重新完成线上导入验收，不能把本地通过当成线上已生效。
